...
- 若规则发生冲突（如图片居中与白边厚度冲突），**优先满足文字侧白边**  
  - 首选：图片按**画布基准居中**  
  - 例外：若画布居中导致文字侧白边不足（< 2.5 × 字高），允许打破居中以满足白边
...
|区域|比例|
|---|---|
|文字所在方向白边|≥ 2.5 × 字高（其中 1 × 为文字，1.5 × 为额外空白）|
|非文字方向白边|0–2%（可为 0）|
...
- 文字 **在满足边界安全距离前提下尽可能靠近照片内容边界**
    
- 文字靠近照片一侧的内边距最小化（文字贴近图片边缘，但需满足边界安全距离）
    
- 图片**以画布为基准居中放置**，不通过移动图片来贴近文字  
  - 若居中导致文字侧白边不足（< 2.5 × 字高），允许打破居中以满足白边
...
- 右侧文字分为上下两段：
    ...
    - 上方：日期（在满足边界安全距离前提下贴近图片上沿）
        
    - 下方：地点 ｜ 描述（在满足边界安全距离前提下贴近图片下沿）
        
    - 两段间距 **明显拉开**（至少 1 × 字高）
        
- 文字列 **贴近图片边缘**，靠近图片一侧内边距最小化
- 图片在画布中 **居中**（以画布为基准）：
