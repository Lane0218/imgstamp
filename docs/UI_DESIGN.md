...
|  (Source Explorer)  |      (Live Preview)           |    (Inspector Panel)        |
...
- **文件操作**：
  ...
  - `📂 打开项目`
- **输出设置** (影响预览与导出)：
  - `📏 目标尺寸`：下拉选择 `5寸 (12.7x8.9cm)` / `6寸`。
- **导出操作**：
  - `🚀 导出成品` (高亮/主色调按钮)

### 3.2 左侧：资源管理器 (Source Explorer)
以垂直滚动列表形式展示源图片。
- **布局**：单列或双列网格。
- **卡片内容**：
  - 小缩略图。
...
- **Ctrl/Shift + 点击**：多选（支持批量编辑，中间预览区显示该批次的第一张图）。
  - **右键菜单**：`移除`、`打开所在文件夹`。
  - **布局切换**：提供单列/两列/三列/四列视图切换，默认两列。

### 3.3 中间：实时预览区 (Live Preview)
**核心体验区域**，所见即所得。
- **画布**：
  - 背景色：中灰色 (#E0E0E0) 或深色，突出照片主体。
  - **实时渲染**：显示当前照片叠加了白边、日期、地点、描述后的最终效果。
  - **渲染逻辑**：
    - 默认生成预览图，渲染方式后续确定（需保证与导出尽量一致）。
    - 当右侧数据变更时，触发预览更新。
  - **控制条 (底部悬浮)**：
    - `🔍 放大/缩小`。
    - `👁️ 查看原图` (按住对比)。
...
- **加载状态**：
  - 图片加载时显示骨架屏 (Skeleton) 或 Loading 转圈。

## 5. 组件库选型 (Ant Design)
- **Layout**: `Layout`, `Sider`, `Content`, `Header`, `Footer`
- **Input**: `DatePicker`, `Input`, `AutoComplete`, `Switch`
- **Feedback**: `Message` (全局提示), `Modal` (导出/大图), `Spin` (加载)
- **Icons**: `@ant-design/icons` (使用通用图标如 `FolderOpenOutlined`, `SaveOutlined`)
